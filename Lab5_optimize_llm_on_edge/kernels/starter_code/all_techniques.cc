...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    assert(A->column % block_size == 0);
    assert((A->column / block_size) % 4 == 0);
   ...
   int cols_per_thread = (C->column + num_thread - 1) / num_thread;
   // Thread creation
   for (int j = 0; j < num_thread; j++) {
       threads_args[j].start_j = j * cols_per_thread;
       threads_args[j].end_j = std::min((j + 1) * cols_per_thread, C->column);
       threads_args[j].params = params;

       pthread_create(
           &thread_pool[j],
           NULL,
           all_techniques_worker_func,
           &threads_args[j]
       );
   }
