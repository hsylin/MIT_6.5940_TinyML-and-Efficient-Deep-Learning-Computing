        ...
        for (int col = mat_args->start; col < mat_args->end; col += 4) {
            ...
            for (int ch = 0; ch < k;) {
                ...
                for (int qj = 0; qj < 16; qj++) 
                {
                    ...
                    uint8_t packed0 = w0_int4[qj];
                    uint8_t packed1 = w1_int4[qj];
                    uint8_t packed2 = w2_int4[qj];
                    uint8_t packed3 = w3_int4[qj];
                
                    signed char w0_de_0  = (packed0 & 0x0F) - 8;
                    signed char w0_de_16 = (packed0 >> 4) - 8;
                
                    signed char w1_de_0  = (packed1 & 0x0F) - 8;
                    signed char w1_de_16 = (packed1 >> 4) - 8;
                
                    signed char w2_de_0  = (packed2 & 0x0F) - 8;
                    signed char w2_de_16 = (packed2 >> 4) - 8;
                
                    signed char w3_de_0  = (packed3 & 0x0F) - 8;
                    signed char w3_de_16 = (packed3 >> 4) - 8;


                    // int8 multiply and accumulate operation
                    signed char a0  = a_int8[qj];
                    signed char a16 = a_int8[qj + 16];
                
                    intermediate_sum0 += a0 * w0_de_0;
                    intermediate_sum0 += a16 * w0_de_16;
                
                    intermediate_sum1 += a0 * w1_de_0;
                    intermediate_sum1 += a16 * w1_de_16;
                
                    intermediate_sum2 += a0 * w2_de_0;
                    intermediate_sum2 += a16 * w2_de_16;
                
                    intermediate_sum3 += a0 * w3_de_0;
                    intermediate_sum3 += a16 * w3_de_16;
                }
...
void MatmulOperator::mat_mul_multithreading_loop_unrolling(struct matmul_params *params) {
   ...
   // Split by groups of 4 columns
   int num_col_groups = n / 4;
   int groups_per_thread = (num_col_groups + num_thread - 1) / num_thread;

   // Thread creation
   for (int j = 0; j < num_thread; j++) {
       int start_group = j * groups_per_thread;
       int end_group = std::min((j + 1) * groups_per_thread, num_col_groups);

       threads_args[j].start = start_group * 4;
       threads_args[j].end = end_group * 4;
